    ...
    while (nprocs--) {
        ...
        while (0 < kval_cnt) {
            ...
            if (0 == strncmp(ESH_KNAME_PTR(addr), ESH_REGION_INVALIDATED, ESH_KNAME_LEN(ESH_REGION_INVALIDATED))) {
                ...
                if (PMIX_SUCCESS != (rc = pmix_bfrops.unpack(&buffer, &val, &cnt, PMIX_VALUE))) {
                    PMIX_ERROR_LOG(rc);
                    goto done;
                }
                if (PMIX_SUCCESS != (rc = pmix_bfrops.copy((void**)kvs, &val, PMIX_VALUE))) {
                    PMIX_ERROR_LOG(rc);
                    goto done;
                }
...
static int pmix_sm_store(ns_track_elem_t *ns_info, pmix_rank_t rank, pmix_kval_t *kval, rank_meta_info **rinfo, int data_exist)
{
    ...
    PMIX_CONSTRUCT(&buffer, pmix_buffer_t);
    if (PMIX_SUCCESS != (rc = pmix_bfrops.pack(&buffer, kval->value, 1, PMIX_VALUE))) {
        PMIX_ERROR_LOG(rc);
        goto exit;
    }
...
static int _store_data_for_rank(ns_track_elem_t *ns_info, pmix_rank_t rank, pmix_buffer_t *buf)
{
    ...
    while (PMIX_SUCCESS == (rc = pmix_bfrops.unpack(buf, kp, &(int){1}, PMIX_KVAL))) {
        pmix_output_verbose(2, pmix_globals.debug_output,
